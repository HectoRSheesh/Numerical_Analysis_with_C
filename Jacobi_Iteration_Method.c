...
// Function to calculate the new value of x1 based on the current values of x2 and x3
float f1(float a, float b, float c){
    return (30 - (2 * b + 3 * c)) / 8;  // Update formula for x1
}

// Function to calculate the new value of x2 based on the current values of x1 and x3
float f2(float a, float b, float c){
    return (1 - (a + 2 * c)) / -9;  // Update formula for x2
}

// Function to calculate the new value of x3 based on the current values of x1 and x2
...
int main(void){
    ...
    do{
        ...
        // Print the approximate values of x1, x2, and x3 at each step
        printf("%d. step approximate values= (%.4f, %.4f, %.4f)\n", i, x1, x2, x3);
    }while ((fabs(x1 - temp1) > hata) || (fabs(x2 - temp2) > hata) || (fabs(x3 - temp3) > hata));  // Continue until the error for all values is within tolerance
